...
namespace Boids
{
    ...
    /// Boidsシミュレーション(2D)
    /// </summary>
    public class Boids2DSimulation : BoidsSimulationBase
    {
        /// <summary>
        /// Boid基本データ構造体
        /// </summary>
        ...
        /// シミュレーション範囲 中心位置
        /// </summary>
        [SerializeField] private Vector2 _simulationAreaCenter = Vector3.zero;
        public Vector2 SimulationAreaCenter => _simulationAreaCenter;

        /// <summary>
        /// シミュレーション範囲 サイズ
        /// </summary>
        [SerializeField]
        private Vector2 _simulationAreaSize = new Vector2(32f, 32f);
        public Vector2 SimulationAreaSize => _simulationAreaSize;
        ...
        protected override void InitBuffer()
        {
            ...
            _boidsForceBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, MaxBoidsNum, Marshal.SizeOf(typeof(Vector2)));

            // データを初期化して設定
            var dataCount = MaxBoidsNum;
            var boidsDataArray = new BoidData[dataCount];
            var boidsForceArray = new Vector2[dataCount];
            for (var i = 0; i < dataCount; i++)
            {
                // 範囲内からランダムに設定
                boidsDataArray[i].Position = UnityEngine.Random.insideUnitCircle * 1.0f;
                boidsDataArray[i].Velocity = UnityEngine.Random.insideUnitCircle * 0.5f;
                boidsForceArray[i] = Vector2.zero;
            }
